...
namespace CircuitBreakerUnitTest
{
    public class CircuitBreakerTest
    {
		...
		{
            Assert.Throws<ArgumentOutOfRangeException>(() => new BaseCircuitBreaker.CircuitBreaker(0, TimeSpan.FromMinutes(5)));
		}

        [Fact]
		public void GivenCircuitBreaker_WhenConstructorWithInvalidTimeOut_ThenThrowException()
		{
            Assert.Throws<ArgumentOutOfRangeException>(() => new BaseCircuitBreaker.CircuitBreaker(1, TimeSpan.Zero));
        }
		...
		{
            // Arrange & Act
			var circuitBreaker = new BaseCircuitBreaker.CircuitBreaker(5, TimeSpan.FromMinutes(5));
